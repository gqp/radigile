...
@section('title', "Join {$teamName} on Radagile")
...
@section('content')
    ...
    <p style="margin:0 0 20px;">
        Someone has invited you to join <strong>{{ $teamName }}</strong> on Radagile as
        a <strong>{{ ucfirst($role) }}</strong>.
    </p>

    @if ($inviteOnly)
        <x-email.callout>
            <strong>Radagile is currently invite-only.</strong>
            Your invitation link below includes an access code so you can create your account.
        </x-email.callout>
    @endif

    <p style="margin:0 0 24px;">To accept, you'll need to create a free Radagile account first — it only takes a minute.</p>
